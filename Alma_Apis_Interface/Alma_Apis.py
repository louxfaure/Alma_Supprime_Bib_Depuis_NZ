...
RESOURCES = {
    ...
    'get_job' : 'conf/jobs/{job_id}/instances?limit=10&offset=0&submit_date_from={date_from}&submit_date_to={date_to}',
...
class Alma(object):
    ...
    #Retrouve tous les traitements passés un même jour un traitement à l'aide de l'identifiant et l'instance du traitement
    def get_job(self, job_id, date_from, date_to, accept='json'):
        response = self.request('GET', 'get_job',
                                {'job_id': job_id,
                                 'date_from': date_from,
                                 'date_to' : date_to},
                                accept=accept)
